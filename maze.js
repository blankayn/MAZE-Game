...
for (let i = 0; i < rows * columns; i++) {
  ...
  if (obstacles.some(obstacle => obstacle.x === cellX && obstacle.y === cellY)) {
    cell.classList.add('obstacle'); // Add obstacle class to designated cells
  }
...
document.addEventListener('keydown', (event) => {
 ...
 let steps = 0;
  ...
  // Check if the character reached the finish line
  if (currentX === columns - 1 && currentY === rows - 1) {
    alert(`Congratulations! You reached the finish line in ${steps} steps.`);
